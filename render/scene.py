...
BEAN_POSES = []
if BEAN_HISTORY:
    # Сыпание начинается не сразу: камера должна успеть подойти и застать
    # именно падение. Раньше зерно укладывалось до её прихода, и большую часть
    # блока она разглядывала уже неподвижную горсть.
    _t = clamp((SCROLL - 0.085) / 0.27, 0.0, 1.0)
